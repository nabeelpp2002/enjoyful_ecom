        ...
        <div className="bg-[var(--color-brand-sand)] min-h-screen">
            <div className="max-w-7xl mx-auto px-4 md:px-8 py-12">
                ...
                <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6 }}>
                    <h1 className="editorial-title display-md mt-12 text-[32px] text-[var(--color-brand-onyx)] tracking-tight">Your Cart</h1>
                </motion.div>
                ...
                {cart.length === 0 ? (
                    ...
                    >
                        ...
                        <h2 className="mb-4 editorial-section-heading text-2xl text-[var(--color-brand-onyx)] tracking-tight">Your cart is empty</h2>
                        <p className="mb-8 editorial-body text-[var(--color-brand-onyx)]/60">Looks like you haven&apos;t added anything yet. Start browsing our collection.</p>
                        <Link href="/category/all">
                            <motion.button
                                whileHover={{ y: -2 }}
                                whileTap={{ scale: 0.98 }}
                                className="inline-block px-8 py-4 rounded-full bg-[var(--color-brand-mustard)] text-[var(--color-brand-onyx)] editorial-heading font-bold shadow-[0_8px_20px_rgba(244,180,73,0.2)]"
                            >
                                Start Shopping
                            </motion.button>
                        </Link>
                    ...
                    <div className="grid grid-cols-1 lg:grid-cols-3 gap-12 mt-8">
                        ...
                        <div className="lg:col-span-2 space-y-6">
                            ...
                            {showProductPrices && (
                                <div className="bg-white rounded-2xl p-4 border border-[var(--color-brand-onyx)]/5">
                                    {shippingRemaining === 0 ? (
                                        <p className="text-sm editorial-subtitle text-green-600">
                                    ...
                                    <div className="flex-1 flex flex-col justify-between py-1">
                                        <div className="flex flex-col sm:flex-row sm:justify-between items-start gap-1">
                                            <div>
                                                <Link href={`/product/${item.product.id}`} className="hover:underline">
                                                    <h3 className="editorial-heading font-bold text-base sm:text-xl text-[var(--color-brand-onyx)] line-clamp-2">{item.product.name}</h3>
                                                </Link>
                                                <p className="editorial-body text-xs sm:text-sm text-[var(--color-brand-onyx)]/60 mt-0.5">
                                            ...
                                            {showProductPrices && (
                                                <Price
                                                    ...
                                                    amountClassName="editorial-number font-bold text-base sm:text-xl text-[var(--color-brand-onyx)]"
                                                    currencyClassName="editorial-number font-bold text-base sm:text-xl text-[var(--color-brand-onyx)]"
                                                />
                                            )}
                                        </div>
                                        <div className="flex justify-between items-end mt-4">
                                            <div className="flex items-center border border-[var(--color-brand-onyx)]/10 rounded-full bg-white">
                                                ...
                                                <span className="w-6 sm:w-8 text-center editorial-heading font-medium text-xs sm:text-sm text-[var(--color-brand-onyx)]">{item.quantity}</span>
                                                <button
                                                    onClick={() => updateCartQuantity(item.product.id, item.quantity + 1)}
                                                    className="w-8 h-8 sm:w-10 sm:h-10 flex items-center justify-center text-[var(--color-brand-onyx)]/60 hover:text-[var(--color-brand-onyx)] transition-colors"
                                                >
                        ...
                        <div className="lg:col-span-1">
                            ...
                            >
                                <h3 className="editorial-heading font-bold text-2xl text-[var(--color-brand-onyx)] mb-6">Order Summary</h3>
                                {showProductPrices ? (
                                    <>
                                        ...
                                        <motion.button
                                            ...
                                            whileHover={{ y: -2 }}
                                            whileTap={{ scale: 0.98 }}
                                            className="w-full py-4 rounded-full bg-[#25D366] text-white editorial-heading font-bold text-base shadow-[0_8px_20px_rgba(37,211,102,0.25)] flex items-center justify-center gap-2"
                                        >
