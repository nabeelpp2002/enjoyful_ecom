...
import { useData, normalizeApiProduct } from "@/context/DataContext";
...
export default function ProductDetail({ params }: { params: Promise<{ id: string }> }) {
    ...
    const { addToWishlist, removeFromWishlist, isInWishlist, products, productsLoading, addToCart, showProductPrices } = useData();
    const [selectedImage, setSelectedImage] = useState(0);
    const [quantity, setQuantity] = useState(1);
    const [openAccordion, setOpenAccordion] = useState<string | null>("benefits");
    const [sizeVariants, setSizeVariants] = useState<SizeVariant[]>([]);
    const [fetchedProduct, setFetchedProduct] = useState<Product | null>(null);
    // Start TRUE: on a fresh mount/navigation the id-or-slug fetch hasn't run yet.
    // If this started false, a product that isn't in the in-memory `products` list
    // (e.g. a non-representative size variant) would hit notFound() on the first
    // render — before the fetch could resolve it. That was the 500ml-variant 404.
    const [resolving, setResolving] = useState(true);
    const [resolveError, setResolveError] = useState(false);
    // The currently-selected size variant (in-place — switching does NOT navigate).
    const [activeVariantId, setActiveVariantId] = useState<string | null>(null);

    // The in-memory list holds the collapsed family representatives. The URL param can be
    // a family slug (group URL), a Mongo _id, or a product slug — match any of them so we
    // can show an instant placeholder (no spinner) while the full document loads.
    const localProduct = products.find(
        (p) => p.id === productId || p.slug === productId || p.productFamily === productId,
    );
    const product = fetchedProduct ?? localProduct;

    useEffect(() => {
        let cancelled = false;
        setFetchedProduct(null);            // drop the previous product's full data
        setResolveError(false);
        setResolving(true);

        const isObjectId = /^[0-9a-fA-F]{24}$/.test(productId);

        // Fetch a FULL populated document (category populated) by id or slug.
        const fetchFull = async (idOrSlug: string) => {
            const byId = /^[0-9a-fA-F]{24}$/.test(idOrSlug);
            const ep = byId ? `/api/products/${idOrSlug}` : `/api/products/slug/${encodeURIComponent(idOrSlug)}`;
            const r = await fetch(ep);
            if (!r.ok) return { ok: false, status: r.status, product: null as Product | null };
            const body = await r.json();
            const raw = body?.data ?? body;
            return { ok: true, status: 200, product: raw && (raw._id || raw.id) ? normalizeApiProduct(raw) : null };
        };

        (async () => {
            try {
                // 1. The param may be a FAMILY slug (group URL). Try the family endpoint first
                //    (only when it's not an ObjectId — ids are never families).
                if (!isObjectId) {
                    const famR = await fetch(`/api/products/family/${encodeURIComponent(productId)}`);
                    if (famR.ok) {
                        const fb = await famR.json();
                        const items: SizeVariant[] = Array.isArray(fb?.data) ? fb.data : Array.isArray(fb) ? fb : [];
                        if (items.length > 0) {
                            const sorted = [...items].sort((a, b) => (a.price ?? 0) - (b.price ?? 0));
                            const def = sorted[0];
                            const full = await fetchFull(def._id);
                            if (cancelled) return;
                            if (full.ok && full.product) {
                                setFetchedProduct(full.product);
                                setActiveVariantId(def._id);
                                setResolving(false);
                                return;
                            }
                        }
                ...
                // 2. Resolve as a single product (by id or slug).
                const single = await fetchFull(productId);
                if (cancelled) return;
                if (single.ok) {
                    setFetchedProduct(single.product);
                    setActiveVariantId(single.product?.id ?? null);
                } else if (single.status === 404) {
                    setFetchedProduct(null);   // genuinely missing → notFound() below
                } else {
                    setResolveError(true);     // transient (API cold/5xx) → retry UI
                }
            } catch {
    ...
    // Fetch sibling size variants when the product has a productFamily set
    useEffect(() => {
        if (!product?.productFamily) { setSizeVariants([]); return; }
        fetch(`/api/products/family/${encodeURIComponent(product.productFamily)}`)
            .then(r => r.ok ? r.json() : null)
            .then(data => {
                const items: SizeVariant[] = Array.isArray(data?.data) ? data.data : Array.isArray(data) ? data : [];
                // Only show selector when there are multiple sizes
                setSizeVariants(items.length > 1 ? items : []);
            })
            .catch(() => setSizeVariants([]));
    }, [product?.productFamily]);

    if (!product) {
        // Still loading the catalogue or resolving this id/slug — don't 404 yet.
        if (resolving || productsLoading) {
            return (
                <div className="bg-[var(--color-brand-sand)] min-h-screen flex items-center justify-center">
                    <div className="w-10 h-10 border-2 border-[var(--color-brand-purple)]/30 border-t-[var(--color-brand-purple)] rounded-full animate-spin" />
                </div>
            );
        }
        // Transient fetch failure (API restarting, network blip) — offer a retry instead
        // of a hard 404, which is reserved for products that genuinely don't exist.
        if (resolveError) {
            return (
                <div className="bg-[var(--color-brand-sand)] min-h-screen flex flex-col items-center justify-center gap-4 px-6 text-center">
                    <p className="font-heading text-lg text-[var(--color-brand-onyx)]">We couldn&apos;t load this product.</p>
                    <p className="font-sans text-sm text-[var(--color-brand-onyx)]/60 max-w-sm">The store may be briefly unavailable. Please try again.</p>
                    <button
